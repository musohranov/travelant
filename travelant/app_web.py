...
def render_map(request: web.Request,
    ...
    #
    # Как вариант (!), на стороне клиента, должны осуществляться все действия необходимые для отображения пользователю.
    # Но возникли проблемы при установке высоты canvas области на этапе отображения страницы в браузере,
    # поэтому необходимый расчет производим здесь.
    #
